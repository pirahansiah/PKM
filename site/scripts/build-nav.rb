...
# Parse Obsidian-style MOC (notes/site/menus.md) → _data/nav.yml + assets/site-map.md
...
SITE_MD = File.join(ROOT, "notes", "site", "menus.md")
...
def content_url_for_path(path)
  ...
  rel = rel.sub(/\.md\z/i, "")
  url = "/#{rel}"
  url.sub(%r{\A/[Nn]otes}, "/notes")
end

def wiki_index
  @wiki_index ||= begin
    idx = {}
    Dir.glob(File.join(ROOT, "notes", "**", "*")).each do |path|
      ...
      idx[rel.sub(/notes/i, "notes").downcase] = raw
...
def resolve_wiki_raw(ref)
  ...
  canonical_asset_path(normalize_url("/notes/#{ref}"))
end
...
def raw_path_from_url(url)
  ...
  return url if url.match?(%r{\A/notes/}i)
...
def build_hashtag_graph
  ...
  Dir.glob(File.join(ROOT, "notes", "**", "*.md")).each do |path|
    next unless File.file?(path)
