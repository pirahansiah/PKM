...
CONTENTS = Path("/Volumes/4tb/2026-07/myWebsite/notes")
OUTPUT = Path("/Volumes/4tb/2026-07/myWebsite/assets/graph.json")

HUBS = {
    "Product": "/notes/docs/product/",
    "Research": "/notes/docs/research/",
    "Solutions": "/notes/docs/solutions/",
    "Content Hub": "/notes/docs/",
    "Wiki": "/notes/wiki/",
    "Portfolio": "/notes/pkm/use-cases/",
}
...
CATEGORY_COLORS = {
    ...
    "patent": "#A78BFA", "keynote": "#ff6482", "course": "#00c7be",
...
def make_id(url):
    ...
    slug = url.replace('/notes/', '').replace('/', '-').strip('-')
    return slug
...
def main():
    ...
    for md_file in sorted(CONTENTS.rglob("*.md")):
        ...
        url = "/notes/" + str(rel).replace('.md', '/')
        if md_file.name == 'index.md':
            url = "/notes/" + str(rel.parent) + "/"
